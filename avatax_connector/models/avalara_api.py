...
import socket
import string
from suds import client, sax, WebFault, wsse
import tempfile
import urllib2
...
class AvaTaxService:
    ...
    def __init__(self, username, password, url, timeout, enable_log=False):
        # This is the company's Development/Production Account number
        self.username = username
        # Put in the License Key received from AvaTax
    ...
    def enable_log(self):
        ...
        handler = logging.FileHandler(os.path.join(
            tempfile.gettempdir(), 'soap-messages.log'))
    ...
    def my_security(self, username, password):
        """Using username and password to verify user account for API access"""
    ...
    def my_profile(self):
        ...
        profile = sax.element.Element('Profile', ns=profileNameSpace)
        profile.append(sax.element.Element(
    ...
    def service(self, name):
        ...
        try:
            svc = client.Client(url=wsdl_url)
            ...
            svc.set_options(wsse=self.my_security(
                    ...
                    if w_message.Severity == 'Error':
                        if w_message._Name in (
                            ...
                            raise UserError(_(
                                ...
                                ' and AvaTax will make an attempt to compute'
                                ' taxes based on the zip code if "Force'
                                ' Address Validation" is disabled in the'
                                ' Avatax connector configuration.  \n\n Also'
                                ' please ensure that the company address is'
                                ' set and Validated. You can get there by'
                                ' going to Sales->Customers and removing'
                                ' "Customers" filter from the search at the'
                                ' top. Then go to your company contact info'
                                ' and validate your address in the Avatax Tab.'
                            ))
                        elif (w_message._Name == 'UnsupportedCountryError'):
                            raise UserError(_(
                                'AvaTax: Notice\n\n Address Validation for'
                                ' this country not supported. But, Avalara'
                                ' will still calculate global tax rules.'
                            ))
                        else:
                            raise UserError(_('AvaTax: Error: ' +
                                              str(w_message._Name) +
                                              "\n\n"
                                              "Summary: " +
                                              w_message.Summary +
    ...
    def get_tax(
        ...
        """ Create tax request and get tax amount by customer address
            ...
            @request.DetailLevel = 'Document': Document (GetTaxResult) level
                details; TaxLines will not be returned.
            @request.DetailLevel = 'Diagnostic': In addition to Tax level
                details, indicates that the server should
            return information about how the tax was calculated. Intended for
                use only while the SDK is in a development environment.
        """
        lineslist = []
        ...
        # service mode = Automatic/Local/Remote
        ...
        if not origin.Line1:
            raise UserError(_(
                'Please set the Company Address in the partner information and'
                ' validate. We are checking against the first line of the'
                ' address and it\'s empty. \n\n Typically located in'
                ' Sales->Customers, you have to clear "Customers" from search'
                ' filter and type in your own company name. Ensure the address'
                ' is filled out and go to Avatax tab in the partner'
                ' information and validate the address. Save partner update'
                ' when done.'
            ))
    ...
    def is_authorized(self):
        return self.get_result(
            self.taxSvc, self.taxSvc.service.IsAuthorized, 'GetTax,PostTax')
...
class Line:
    ...
    def __init__(self, taxSvc, ItemCode, Amount, Qty,
                 Description=None, TaxCode=None):
        self.taxSvc = taxSvc
        # We're not setting No here
        self.data = self.default_line()
