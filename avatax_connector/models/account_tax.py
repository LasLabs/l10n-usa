...
class AccountTax(models.Model):
    ...
    def _get_compute_tax(
        ...
        if not partner.customer_code:
            if not avatax_config.auto_generate_customer_code:
                raise UserError(_(
                    ...
                    ' edit the Customer Code in customer profile. You can fix'
                    ' by clicking "Generate Customer Code" button in the'
                    ' customer contact information.' % partner.name
                ))
            else:
        ...
        # this condition is required, in case user select force address
        # validation on AvaTax API Configuration
    ...
    def cancel_tax(self, avatax_config, doc_code, doc_type, cancel_code):
        """Used to cancel when there is no need for tax calculation"""
        avalara_obj = AvaTaxService(
            avatax_config.account_number,
            avatax_config.license_key,
            avatax_config.service_url,
