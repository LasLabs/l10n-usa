...
class SaleOrder(models.Model):
    ...
    tax_amount = fields.Float(
        'Tax Code Amount', digits_compute=dp.get_precision('Sale Price'))
    tax_add_default = fields.Boolean('Default Address', readonly=True, states={
                                     'draft': [('readonly', False)]})
    tax_add_invoice = fields.Boolean('Invoice Address', readonly=True, states={
                                     'draft': [('readonly', False)]})
    tax_add_shipping = fields.Boolean(
        'Delivery Address', default=True, readonly=True, states={
            'draft': [
                ('readonly', False)]})
    tax_add_id = fields.Many2one(
        'res.partner', 'Tax Address', readonly=True, states={
            'draft': [
                ('readonly', False)]})
    tax_address = fields.Text()
    ...
    def _onchange_tax_add_default_partner_id(self):
    ...
    def _onchange_tax_add_invoice_partner_invoice_id_partner_id(self):
        if (self.tax_add_invoice and self.partner_invoice_id) or (
                self.tax_add_invoice and self.partner_id):
    ...
    def _onchange_tax_add_shipping_partner_shipping_id_partner_id(self):
        if (self.tax_add_shipping and self.partner_shipping_id) or (
                self.tax_add_shipping and self.partner_id):
    ...
    def write(self, vals):
        for self_obj in self:
            ...
            if 'tax_add_default' in vals and vals['tax_add_default']:
                ...
                ship_add_id = self_obj.partner_invoice_id \
                    or self_obj.partner_id
            elif 'tax_add_shipping' in vals and vals['tax_add_shipping']:
                ship_add_id = self_obj.partner_shipping_id \
                    or self_obj.partner_id
    ...
    def create_lines(self, order_lines):
        """Tax line creation for calculating tax amount using avalara code."""
        lines = []
        for line in order_lines:
            if line.product_id and line.product_id.tax_apply:
                ...
                lines.append({
                    ...
                    'itemcode': line.product_id
                    and line.product_id.default_code or None,
    ...
    def compute_tax(self):
        """Create and update tax amount for each and every order and shipping line.
        @param order_line: send sub_total of each line and get tax amount
        @param shiiping_line: send shipping amount of each ship line and get
            ship tax amount
        """
        ...
        if not avatax_config:
            raise UserError(_(
                'Your Avatax Countries settings are not configured. You need a'
                ' country code in the Countries section. \n If you have a'
                ' multi-company installation, you must add settings for each'
                ' company. \n\n You can update settings in Avatax->Avatax API.'
            ))

        tax_amount = o_tax_amt = 0.0

        # ship from Address / Origin Address either warehouse or company if
        # none
            ...
            if lines:
                if avatax_config.on_line:
                    ...
                    for line in lines:
                        ...
                        # tax amount based on total order line total
    ...
    def button_dummy(self):
        """Used to call manual avalara calculation method and get tax amount"""
        self.compute_tax()
